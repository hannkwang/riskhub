    ...
    <Card padding={false} className={`overflow-hidden transition-shadow hover:shadow-md ${item.slaBreached ? 'border-red-200' : ''}`}>
      ...
      {expanded && (
        <div className="border-t border-slate-100 p-5 bg-slate-50 space-y-4">
          {item.risk_statement && (
            <div>
              <div className="text-xs font-medium text-slate-400 uppercase tracking-wider mb-1">Risk Description</div>
              <p className="text-sm text-slate-700 leading-relaxed">{item.risk_statement}</p>
            </div>
          )}

          {/* Scores */}
          <div className={`grid gap-3 ${item.residual_score ? 'grid-cols-2' : 'grid-cols-1'}`}>
            <div className="bg-white rounded-lg border border-slate-200 p-3">
              <div className="text-xs text-slate-500 mb-1">Inherent risk</div>
              <RiskBadge level={item.level} />
              <div className="text-xs text-slate-400 mt-1 font-mono">{item.score} · I:{item.impact} · L:{item.likelihood}</div>
            </div>
            {item.residual_score && (
              <div className="bg-white rounded-lg border border-slate-200 p-3">
          ...
          {item.mitigations && item.mitigations.length > 0 && (
            <div>
              <div className="text-xs font-medium text-slate-400 uppercase tracking-wider mb-2">
                Mitigations · {item.mitigations.length} item{item.mitigations.length !== 1 ? 's' : ''}
              </div>
              <ul className="space-y-1.5">
                {item.mitigations.map((m, i) => (
                  <li key={i} className="flex items-start gap-2 text-sm bg-white rounded-lg border border-slate-200 px-3 py-2">
                    <Check size={13} className="text-emerald-500 mt-0.5 flex-shrink-0" />
                    <span className="text-slate-700 flex-1">{m.text}</span>
                    <span className="text-xs text-slate-400 flex-shrink-0 capitalize">{m.type}</span>
          ...
          {item.justification && (
            <div>
              ...
              <p className="text-sm text-slate-700 leading-relaxed">{item.justification}</p>
            </div>
          )}
