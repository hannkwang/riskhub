...
function seedIfEmpty() {
  ...
  [
    ['mira',    'Mira Tanaka',   'engineer',       'App / Payments'],
    ['sam',     'Sam Okafor',    'engineer',       'App / Identity'],
    ['devon',   'Devon Reyes',   'engineer',       'Infra / Cloud'],
    ['lee',     'Lee Park',      'engineer',       'Infra / Storage'],
    ['jordan',  'Jordan Walsh',  'biz_owner',      'Payments BU'],
    ['sara',    'Sara Lin',      'biz_owner',      'Identity BU'],
  ...
  [
    ['sys-1', 'Payments Gateway',        'Critical', 'Restricted',   1, 'Jordan Walsh', 'Payments BU',   'High'],
    ['sys-2', 'Vendor SFTP Gateway',     'High',     'Restricted',   1, 'Jordan Walsh', 'Payments BU',   'High'],
    ['sys-3', 'Identity Provider (SSO)', 'Critical', 'Restricted',   1, 'Sara Lin',     'Identity BU',   'High'],
    ['sys-4', 'HR Workday',              'High',     'Confidential', 0, 'Marcus Webb',  'People Ops',    'Medium'],
    ['sys-5', 'Data Lake — Customer',    'Critical', 'Restricted',   0, 'Lia Romero',   'Data Platform', 'High'],
    ['sys-6', 'Customer Support Portal', 'Medium',   'Confidential', 1, 'Tomás Field',  'Support BU',    'Medium'],
    ['sys-7', 'Internal Wiki',           'Low',      'Internal',     0, 'Anita Cole',   'IT Ops',        'Low'],
  ...
  const seedRisks = [
    // --- Concurrent Review (all concurrent approvers working in parallel) ---
    {
      ...
      owner: 'Mira Tanaka', createdBy: 'mira', team: 'App / Payments', system: 'Mobile Banking App',
      impact: 5, likelihood: 4, ri: 3, rl: 2,
      stage: 'Concurrent Review',
      mitigations: JSON.stringify([
        { text: 'Implement 5-attempt lockout with 15-min progressive delay', owner: 'Mira', due: 'Jun 30', type: 'preventive' },
        { text: 'Push notification alert on PIN failure >3 attempts', owner: 'Ravi', due: 'Jun 30', type: 'detective' },
        { text: 'Biometric fallback mandatory for high-value transactions', owner: 'Sara', due: 'Jul 31', type: 'preventive' },
      ]),
      justification: 'Account lockout and biometric requirements reduce brute-force likelihood to near-zero. Residual risk is Low — accepted by Retail BU pending Jul 31 biometric rollout. Named owner: Sara Lin. Monitoring KPI: >3 PIN failures in 10 min triggers SOC alert. Reassessment at biometric rollout completion.',
      reviewMonths: 6,
      createdAt: daysAgo(18), updatedAt: daysAgo(3), expiresAt: null,
      history: [
        { from: null,             to: 'Draft',             actor: 'mira',   name: 'Mira Tanaka',   action: 'create',  comment: 'Draft created', at: daysAgo(18) },
        { from: 'Draft',          to: 'System Owner',       actor: 'mira',   name: 'Mira Tanaka',   action: 'submit',  comment: 'Lockout gap confirmed by AppSec pen-test finding PT-2026-089.', at: daysAgo(12) },
        { from: 'System Owner',   to: 'Concurrent Review',  actor: 'sara',   name: 'Sara Lin',      action: 'approve', comment: 'High priority — fast-tracked to concurrent review. Biometric deadline accepted.', at: daysAgo(3) },
      ],
    },
    {
      id: 'RA-2026-004',
      ...
      owner: 'Sam Okafor', createdBy: 'sam', team: 'App / Identity', system: 'Data Lake — Customer',
      impact: 5, likelihood: 3, ri: 3, rl: 2,
      stage: 'Concurrent Review',
      mitigations: JSON.stringify([
        { text: 'Apply column-level masking on NRIC, address, income fields in all Athena views', owner: 'Sam', due: 'Jun 20', type: 'preventive' },
        { text: 'Audit and revoke data-read IAM role from non-data-platform principals', owner: 'Devon', due: 'Jun 15', type: 'preventive' },
        { text: 'Enable CloudTrail data-event logging on S3 buckets backing the Data Lake', owner: 'Ravi', due: 'Jun 15', type: 'detective' },
        { text: 'Monthly IAM access review for Data Lake roles', owner: 'Sam', due: 'Ongoing', type: 'detective' },
      ]),
      justification: 'Column-level masking eliminates direct PII access. IAM remediation reduces scope to data-platform team only. Residual risk is Medium — accepted by Lia Romero (Data Platform lead) pending masking deployment. Monitoring KPIs: CloudTrail alerts on mass-select queries (>10k rows/min) and IAM policy change events. Quarterly IAM review cadence.',
      reviewMonths: 6,
      createdAt: daysAgo(10), updatedAt: daysAgo(1), expiresAt: null,
      history: [
        { from: null,            to: 'Draft',             actor: 'sam',    name: 'Sam Okafor',   action: 'create',  comment: 'Draft created', at: daysAgo(10) },
        { from: 'Draft',         to: 'System Owner',       actor: 'sam',    name: 'Sam Okafor',   action: 'submit',  comment: 'IAM misconfiguration identified during Q2 access review. Masking remediation scoped.', at: daysAgo(6) },
        { from: 'System Owner',  to: 'Concurrent Review',  actor: 'sara',   name: 'Sara Lin',     action: 'approve', comment: 'Data Platform owner Lia Romero briefed. Approved to proceed to concurrent review.', at: daysAgo(1) },
      ],
    },
    ...
    // --- System Owner stage ---
    {
      ...
      owner: 'Devon Reyes', createdBy: 'devon', team: 'Infra / Cloud', system: 'Identity Provider (SSO)',
      impact: 5, likelihood: 3, ri: 2, rl: 2,
      stage: 'System Owner',
      mitigations: JSON.stringify([
        { text: 'Upgrade SAML library to python3-saml ≥3.0.0 with XSW protection', owner: 'Devon', due: 'Jul 15', type: 'preventive' },
        { text: 'Add integration test suite for XSW attack vectors (OWASP list)', owner: 'Devon', due: 'Jul 20', type: 'preventive' },
        { text: 'Enable anomaly alerting on SAML login events from unusual source IPs', owner: 'Ravi', due: 'Jun 30', type: 'detective' },
      ]),
      justification: 'Library upgrade eliminates the XSW vulnerability class. Residual risk is Low after upgrade. Exploit requires existing authenticated session — opportunistic exploitation is difficult. Named owner: Sara Lin (IdP). Monitoring KPI: SIEM alert on SSO logins outside business hours from new IPs. Reassessment post-upgrade in Aug 2026.',
      reviewMonths: 6,
      createdAt: daysAgo(5), updatedAt: daysAgo(1), expiresAt: null,
      history: [
        { from: null,    to: 'Draft',        actor: 'devon', name: 'Devon Reyes', action: 'create', comment: 'Draft created', at: daysAgo(5) },
        { from: 'Draft', to: 'System Owner', actor: 'devon', name: 'Devon Reyes', action: 'submit', comment: 'Finding from external pen-test (Coalfire, May 2026). CVSS 8.8. Library upgrade scheduled for Jul.', at: daysAgo(1) },
      ],
    },
    ...
    // --- Approved ---
    {
      ...
      owner: 'Mira Tanaka', createdBy: 'mira', team: 'App / Payments', system: 'Payments Gateway',
      impact: 5, likelihood: 3, ri: 2, rl: 1,
      stage: 'Approved',
      mitigations: JSON.stringify([
        { text: 'Disable debug logging in production Payments Gateway (immediate)', owner: 'Mira', due: 'May 20', type: 'preventive' },
        { text: 'Implement PAN tokenisation before any log write in payment flow', owner: 'Mira', due: 'Jun 30', type: 'preventive' },
        { text: 'Restrict Splunk access to PCI-scoped indexes for Tier-2 ops', owner: 'Ravi', due: 'Jun 15', type: 'preventive' },
        { text: 'DLP rule in Splunk to detect and alert on PAN-formatted strings in logs', owner: 'Ravi', due: 'Jun 15', type: 'detective' },
      ]),
      justification: 'Debug logging disabled immediately on detection — no new PANs entering logs. Tokenisation eliminates the root cause permanently. DLP alerting provides ongoing detective control. Residual risk is Very Low post-remediation. Business owner: Jordan Walsh (Payments BU). Monitoring KPI: DLP alert triggers must be zero within 14 days of tokenisation go-live.',
      reviewMonths: 3,
      createdAt: daysAgo(30), updatedAt: daysAgo(8), expiresAt: isoFuture(82),
      history: [
        { from: null,              to: 'Draft',             actor: 'mira',    name: 'Mira Tanaka',   action: 'create',  comment: 'Draft created', at: daysAgo(30) },
        { from: 'Draft',           to: 'System Owner',      actor: 'mira',    name: 'Mira Tanaka',   action: 'submit',  comment: 'P1 incident — debug flag found during PCI audit prep. Immediate disable underway.', at: daysAgo(25) },
        { from: 'System Owner',    to: 'Concurrent Review', actor: 'jordan',  name: 'Jordan Walsh',  action: 'approve', comment: 'Fast-tracked. Payments BU accepts risk pending tokenisation.', at: daysAgo(23) },
    ...
    },
    {
      ...
      owner: 'Lee Park', createdBy: 'lee', team: 'Infra / Storage', system: 'HR Workday',
      impact: 3, likelihood: 4, ri: 2, rl: 2,
      stage: 'Approved',
      mitigations: JSON.stringify([
        { text: 'Automate AD deprovisioning via HRMS webhook on termination event', owner: 'Lee', due: 'Jul 31', type: 'preventive' },
        { text: 'Daily audit job: flag any contractor accounts active >48h post-termination', owner: 'Devon', due: 'Jun 20', type: 'detective' },
        { text: 'Weekly deprovisioning compliance report to HRBP and IT Ops', owner: 'Lee', due: 'Ongoing', type: 'detective' },
      ]),
      justification: 'Automation eliminates the manual gap. Daily audit provides compensating detective control until automation is live. Residual risk is Low — accepted by Marcus Webb (People Ops). Monitoring KPI: zero active contractor accounts >48h post-termination date (tracked via daily audit job). Reassessment after automation go-live, Q3 2026.',
      reviewMonths: 9,
      createdAt: daysAgo(45), updatedAt: daysAgo(14), expiresAt: isoFuture(256),
      history: [
        { from: null,              to: 'Draft',             actor: 'lee',     name: 'Lee Park',      action: 'create',  comment: 'Draft created', at: daysAgo(45) },
        { from: 'Draft',           to: 'System Owner',      actor: 'lee',     name: 'Lee Park',      action: 'submit',  comment: 'Identified during internal audit review cycle Q1 2026.', at: daysAgo(38) },
        { from: 'System Owner',    to: 'Concurrent Review', actor: 'jordan',  name: 'Jordan Walsh',  action: 'approve', comment: 'People Ops briefed. Marcus Webb accepts. Automation scoped.', at: daysAgo(35) },
    ...
    // --- Draft ---
    {
      ...
      owner: 'Devon Reyes', createdBy: 'devon', team: 'Infra / Cloud', system: 'K8s DEV Cluster',
      impact: 3, likelihood: 3, ri: 2, rl: 1,
      stage: 'Draft',
      mitigations: JSON.stringify([
        { text: 'Migrate all secrets to Kubernetes Secrets with RBAC read restrictions', owner: 'Devon', due: 'Jun 30', type: 'preventive' },
        { text: 'Integrate HashiCorp Vault for dynamic credential injection', owner: 'Devon', due: 'Aug 31', type: 'preventive' },
        { text: 'OPA/Gatekeeper policy to block pod specs with base64-pattern env vars at admission', owner: 'Devon', due: 'Jul 15', type: 'preventive' },
      ]),
      ...
      history: [
        { from: null, to: 'Draft', actor: 'devon', name: 'Devon Reyes', action: 'create', comment: 'Draft created — found during K8s security review.', at: daysAgo(2) },
      ],
    },
    ...
    // --- Rejected ---
    {
      ...
      owner: 'Lee Park', createdBy: 'lee', team: 'Infra / Storage', system: 'Customer Support Portal',
      impact: 2, likelihood: 4, ri: 2, rl: 2,
      stage: 'Rejected',
      mitigations: JSON.stringify([
        { text: 'Return identical response for valid and invalid email addresses', owner: 'Lee', due: 'Jul 1', type: 'preventive' },
      ]),
      justification: 'Single mitigation adequately addresses the issue.',
      reviewMonths: 6,
      createdAt: daysAgo(20), updatedAt: daysAgo(7), expiresAt: null,
      history: [
        { from: null,           to: 'Draft',        actor: 'lee',    name: 'Lee Park',     action: 'create',          comment: 'Draft created', at: daysAgo(20) },
        { from: 'Draft',        to: 'System Owner', actor: 'lee',    name: 'Lee Park',     action: 'submit',          comment: 'Identified during OWASP top 10 review.', at: daysAgo(14) },
        { from: 'System Owner', to: 'Rejected',     actor: 'jordan', name: 'Jordan Walsh', action: 'reject',          comment: 'Justification is insufficient for a Medium inherent risk. Please include: (1) named accountable owner, (2) at least two monitoring KPIs, (3) business rationale for accepting rather than fixing immediately. Re-submit after updating per BP-013.', at: daysAgo(7) },
      ],
    },
  ];
